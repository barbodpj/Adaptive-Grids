...
class DirectMPIGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self.density = grid.create_grid(
                ...
                config=self.density_config,stage=self.stage)
        ...
        if rgbnet_dim <= 0:
            ...
            self.k0 = grid.create_grid(
                ...
                xyz_min=self.xyz_min, xyz_max=self.xyz_max,
                config=self.k0_config,stage=self.stage)
            self.rgbnet = None
        else:
        ...
        if stage == "fine":
            ...
            alpha_W = torch.ones(size=(V,)).to(device="cuda") / sqrt_W
            ...
            self.alpha_uvw = self.alpha_uvw[..., None].expand(-1, -1, -1, 10)
    ...
    def find_important_blocks(self):
        self.density.block_num = torch.floor(self.world_size/self.density.block_size).to(dtype=torch.long) + 1
        self.k0.block_num =self.density.block_num

        importance_list = []
        x=int(self.density.block_size[0]/2)
        for i in range(self.density.block_num[0]):
            for j in range(self.density.block_num[1]):
                for k in range(0, self.density.block_num[2]):
                    ...
                    dct_coefs = self.mathematical_DCT(features)

                    density_coefs = dct_coefs[..., 0]

                    weights_1 = torch.arange(1, x+1)[None, None, :].repeat(x, x, 1)
                    weights_2 = torch.arange(1, x+1)[:, None, None].repeat(1, x, x)
                    weights_3 = torch.arange(1, x+1)[None, :, None].repeat(x, 1, x)
                    weights = torch.sqrt(weights_1 * weights_1 + weights_2 * weights_2 + weights_3 * weights_3)

                    # importance = torch.sum(weights * torch.abs(density_coefs[x:2*x,x:2*x,x:2*x]))

                    color_coefs = dct_coefs[..., 1:]
                    weights = weights[..., None].repeat(1, 1, 1, 9)

                    importance = torch.sum(weights * torch.abs(color_coefs[x:2*x, x:2*x, x:2*x, :]))

                    importance_list.append(([i, j, k], importance))
