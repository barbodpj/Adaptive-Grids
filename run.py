...
def config_parser():
    ...
    parser.add_argument("--eval_ssim", default=False, action='store_true')
    parser.add_argument("--eval_lpips_alex", action='store_true')
    parser.add_argument("--eval_lpips_vgg", default=False, action='store_true')
    ...
    parser.add_argument("--landa", type=float, default=0)
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if gt_imgs is not None and render_factor==0:
            block_size= 200
            qs = []
            for ii in range(0,rgb.shape[0],block_size):
                for jj in range(0, rgb.shape[1],block_size):
                    q = -10. * np.log10(np.mean(np.square(rgb[ii:ii+block_size,jj:jj+block_size] - gt_imgs[i][ii:ii+block_size,jj:jj+block_size])))
                    qs.append(q)
                    # if q < min_q:
                    #     min_q = q
            qs.sort()
            qs = qs[0:5]
            important_psnrs.append(np.mean(qs))
    ...
    if len(psnrs):
        ...
        print('Testing important psnr', np.mean(important_psnrs), '(avg)')
...
def create_new_model(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path):
    ...
    if cfg.data.ndc:
        ...
        model = dmpigo.DirectMPIGO(
            ...
            num_voxels=num_voxels,stage=stage,
            **model_kwargs)
        ...
        model = dvgo.DirectVoxGO(
            xyz_min=xyz_min, xyz_max=xyz_max,
            num_voxels=num_voxels,
            mask_cache_path=coarse_ckpt_path,stage=stage,
            **model_kwargs)
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            ...
            torch.cuda.empty_cache()
        if global_step == 50002 and stage=='fine':
            # a = torch.arange(1,5)
            # b = torch.arange(1,5)
            # c = torch.arange(1,3)
            #
            # indices = torch.meshgrid(a,b,c)
            # ind1 = indices[0].flatten()
            # ind2 = indices[1].flatten()
            # ind3 = indices[2].flatten()
            # model.voxel_size = model.voxel_size/2
            # model.density.grid.requires_grad = False
            # model.k0.grid.requires_grad = False
            # print("requires grad: density and k0" , model.density.grid.requires_grad, model.k0.grid.requires_grad)

            importance_list = model.find_important_blocks()
            ind1 = []
            ind2 = []
            ind3 = []
            for i in range(model.density.important_num):
                block_loc = importance_list[i][0]
                ind1.append(block_loc[0])
                ind2.append(block_loc[1])
                ind3.append(block_loc[2])

            ind1 = torch.tensor(ind1)
            ind2 = torch.tensor(ind2)
            ind3 = torch.tensor(ind3)

            model.density.initialize_importance_subsampling([ind1,ind2,ind3])
            model.k0.initialize_importance_subsampling([ind1,ind2,ind3])
        ...
        # if model.density.voxel_subsampling:
        #     landa = args.landa
        #     new_loss = torch.sqrt(torch.sum(model.density.important_voxels**2)) / (model.density.important_voxels.numel())
        #     new_loss +=  torch.sqrt(torch.sum(model.k0.important_voxels**2)) / (model.k0.important_voxels.numel())
        #     loss += landa/(1+new_loss)
...
if __name__=='__main__':
    ...
    if args.render_test or args.render_train or args.render_video:
        ...
        importance_list = model.find_important_blocks()
        with torch.no_grad():
            block_size = model.density.block_size[0]
            for j in range(0,10):
                block_poisition = importance_list[j][0]
                block_poisition[0] = block_poisition[0]*block_size
                block_poisition[1] = block_poisition[1]*block_size
                block_poisition[2] = block_poisition[2]*block_size
                print(block_poisition)
                model.density.grid[0,0,block_poisition[0]:block_poisition[0]+block_size, block_poisition[1]:block_poisition[1]+block_size,block_poisition[2]:block_poisition[2]+block_size] = 10
                model.k0.grid[0,:,block_poisition[0]:block_poisition[0]+block_size, block_poisition[1]:block_poisition[1]+block_size,block_poisition[2]:block_poisition[2]+block_size] = 0
    ...
    if args.render_train:
        ...
        for i in range(rgbs.shape[0]):
            imageio.imwrite(os.path.join(testsavedir, 'test_image_'+ str(i)+".jpg"),x[i])
